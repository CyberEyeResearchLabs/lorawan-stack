...
func TestRolloverClock(t *testing.T) {
	...
	for i, stc := range []struct {
		...
		{
			// 1 rollover.
			Absolute: ConcentratorTime(math.MaxUint32*time.Microsecond) + ConcentratorTime(5*time.Second),
			Relative: uint32(5000000),
		},
		{
			// 3 rollovers (1 existing + 2 server time rollovers).
			Absolute: ConcentratorTime(3*math.MaxUint32*time.Microsecond) + ConcentratorTime(10*time.Second),
			Relative: uint32(10000000),
		},
		{
			// 5 rollovers (3 existing + 1 concentrator timestamp rollover + 1 server time rollover).
			Absolute: ConcentratorTime(5*math.MaxUint32*time.Microsecond) + ConcentratorTime(1*time.Second),
			Relative: uint32(1000000),
		},
		{
			// 5 rollovers (5 existing) and advance to end of concentrator time epoch.
			Absolute: ConcentratorTime(6*math.MaxUint32*time.Microsecond) - ConcentratorTime(1*time.Second),
			Relative: uint32(4293967295),
		},
	} {
